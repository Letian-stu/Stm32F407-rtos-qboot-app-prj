 ...
 * Change Logs:
 * Date           Author       Notes
 * 2022-05-23     tian       the first version
 */
/*
...
#define DBG_ENABLE
#define DBG_SECTION_NAME          "http_ota_to_factory"
...
static int http_ota_fw_download(const char* uri)
{
    ...
    if ((dl_part = fal_partition_find("factory")) == RT_NULL)
    {
        LOG_E("Firmware download failed! Partition (%s) find error!", "factory");
        ret = -RT_ERROR;
        goto __exit;
    }
...
void http_ota_to_factory(uint8_t argc, char **argv)
{
    if (argc < 2)
    {
 ...
 * msh />http_ota [url]
*/
MSH_CMD_EXPORT(http_ota_to_factory, Use HTTP to factory the firmware);
