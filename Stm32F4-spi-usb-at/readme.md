...
软件包开启fal fatfs
fal使用sfud驱动程序
设置fal设备名称 #define NOR_FLASH_DEV_NAME             "w25q16"
在main中添加 fal_init();
修改 fal_cfg.h中
```c
/*
...
{                                                                                       \
    ...
    {FAL_PART_MAGIC_WORD,        "app",           "onchip_flash",               128*1024,   447*1024, 0}, \
    {FAL_PART_MAGIC_WORD,    "factory",           "onchip_flash",         (128+448)*1024,   447*1024, 0}, \
    {FAL_PART_MAGIC_WORD, "filesystem",       NOR_FLASH_DEV_NAME,                      0,  1024*1024, 0}, \
    {FAL_PART_MAGIC_WORD,   "download",       NOR_FLASH_DEV_NAME,              1024*1024,   512*1024, 0}, \
    {FAL_PART_MAGIC_WORD,  "easyflash",       NOR_FLASH_DEV_NAME,        (1024+512)*1024,   512*1024, 0}, \
}
...
# 4. 目标4 把w25q16模拟u盘 device模式
1.rtt setting 中 使用usb设备
2.使能复合设备
3.将设备作为大容量设备
4.修改磁盘名
5.进入cube setting 配置 usb_otg_fs
usb_otg_fs 全速
usb_otg_hs 高速 需要外挂芯片
//模式选择OTG/Dual_Role_Device
6.配置
```
...
**解决**:发现在drv_usbd.c中有和hal库的接口,没看见HAL_PCD_SuspendCallback函数,就自己加一个
